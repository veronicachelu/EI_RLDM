...
from rnn_lib.init_policies import Bias_ZerosInit, Hidden_ZerosInit, ColumnEiCell_U_InitPolicy, ColumnEiCell_W_InitPolicy, calculate_ColumnEi_layer_params
from rnn_lib.update_policies import ColumnEiSGD_Clip
...
class ColumnEiCell(BaseRNNCell):
    ...
    def __init__(self, n_input, layer_params: '(n_hidden, ratio)', nonlinearity=None, learn_hidden_init=False,
                 ...
                 update_policy=ColumnEiSGD_Clip(),
                 clamp=True):
        """
    ...
    # @property
    def h_i(self, x):
         return torch.mm(self.U, x.T) + torch.mm(self.W[self.ne_h:,:], self.h) + self.b
